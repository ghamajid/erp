        ...
        <div class="invoice_logo" style="width:25%">
            <table class="table-borderless mr_0 ml_auto">
                ...
                <tr>
                    ...
                    <td><span class="p-1">:</span> @if ($data->type == 1) {{ __('common.Regular') }} @elseif ($data->type == 0) {{ __('common.Conditional') }} @else {{ __('common.POS') }}@endif</td>
                </tr>
                <tr>
    ...
    <div class="invoice_info">
        <table class="table table-bordered billing_info">
            <tr>
                ...
                <th scope="col" style="width:15%; text-align: right; padding-left:0">{{__('sale.SubTotal')}}</th>
            </tr>
            ...
            @foreach($data->items as $key=> $item)
                ...
                @if ($item->productable->product)
                    ...
                    <tr>
                        ...
                        <td>{{$item->tax}}%</td>
                        <td>{{$item->discount}}</td>
                        <td style="text-align: right">{{single_price($item->sub_total)}}</td>
                    </tr>
                @else
                    @php
            ...
            <tfoot>
                @php
                    ...
                    $this_due = $data->payable_amount - $data->payments->sum('amount') - $data->payments->sum('advance_amount') - $data->payments->sum('return_amount');
                    $discount = $data->total_discount;
                    $vat =(($data->amount - $discount) * $data->total_tax) / 100;
                @endphp
                @php
                    $paid =0;
                @endphp
            <tr>
                <td @if (app('general_setting')->origin == 1) colspan="9" @else colspan="8" @endif style="text-align: right">
                    <ul>
                        ...
                        @endif
                        @if ($vat > 0)
                            <li>{{__('quotation.Other Tax')}} ({{ $data->total_tax }}%)
                                :
                            </li>
                        @endif
